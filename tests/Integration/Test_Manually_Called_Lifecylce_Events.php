		...
		self::$app_instance
			...
			->module(
				...
				fn( Perique_Migrations $e) => $e
					->set_migration_log_key( 'test_create_and_seed_on_activation' )
					->add_migration( Simple_Table_Migration::class )            // No seed data
					->add_migration( Has_Seeds_Migration::class )               // Has seed data, should create
					->add_migration( Has_Seeds_Migration_But_Disabled::class )  // Has seed data, but should not create.
			)
			->boot();
	...
	public function test_drop_tables_on_deactivate(): void {
		...
		self::$app_instance
			...
			->module(
				Perique_Migrations::class,
				fn( Perique_Migrations $e) => $e
					->set_migration_log_key( 'test_drop_tables_on_deactivate' )
					->add_migration( Drop_On_Deactivation_Migration::class )
					->add_migration( Not_Drop_On_Deactivation_Migration::class )
			)
			->boot();
	...
	public function test_drop_tables_on_uninstall(): void {
		...
		self::$app_instance
			...
			->module(
				Perique_Migrations::class,
				fn( Perique_Migrations $e) => $e
					->set_migration_log_key( 'test_drop_tables_on_uninstall' )
					->add_migration( Drop_On_Uninstall_Migration::class )
					->add_migration( No_Drop_On_Uninstall_Migration::class )
			)
			->boot();
